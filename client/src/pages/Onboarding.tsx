...
// //   // ✅ FIXED: Added missing handleSubmit function definition
...
// //         targetDate: targetAchievementDate // Sending the date we calculated
// //       });
...
// //         className="relative z-10 w-full max-w-2xl bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-2xl shadow-2xl"
// //       >
...
// //           </div>
// //           <p className="text-slate-400">Final step! Let's get your dashboard ready.</p>
// //         </div>
...
// //           <div>
// //             <p className="text-xs font-bold uppercase text-slate-500 tracking-widest mb-1">Your BMI</p>
// //             <p className="text-3xl font-black text-emerald-400">{bmi ? bmi.toFixed(1) : "--"}</p>
// //           </div>
// //           <div>
// //             <p className="text-xs font-bold uppercase text-slate-500 tracking-widest mb-1">Goal</p>
...
// //             </div>
// //             <div>
// //               <p className="text-[10px] font-black uppercase text-emerald-500/70 tracking-widest">Estimated Achievement Date</p>
...
// //             type="submit"
// //             disabled={loading}
// //             className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-800 disabled:text-slate-500 disabled:cursor-not-allowed text-slate-950 font-bold text-lg rounded-xl transition-all shadow-lg shadow-emerald-900/10"
// //           >
// //             {loading ? "Setting up dashboard..." : "Create My Account"}
// //           </Button>
// //         </div>
// //       </form>
// //     </div>
...
// const Onboarding = () => {
//   const navigate = useNavigate();
//   // ✅ Using refreshAppData instead of just fetchProfile
...
//   setLoading(true);
//   try {
//     // 1. Grab token directly from storage
//     const token = localStorage.getItem("token");

//     // 2. Save the profile - This MUST succeed
...
//     // We wrap this in its own try/catch so if it fails, 
//     // it doesn't stop the user from going to the dashboard.
//     try {
//       if (typeof refreshAppData === 'function') {
//         await refreshAppData(token);
//       }
//     } catch (refreshErr) {
//       console.warn("Background data refresh failed, but proceeding to app:", refreshErr);
//     }
    
//     // 4. Success and Redirect
//     toast.success("Fitness profile established!");
    
//     // Use a small delay or immediate navigation
//     setTimeout(() => {
//       navigate("/app");
//     }, 100);
...
// //   setLoading(true);
// //   try {
// //     // 1. Get the token directly from storage to ensure it's fresh
// //     const token = localStorage.getItem("token");
...
// //     // 3. Refresh all data (Profile + Daily Logs) before redirecting
// //     await refreshAppData(token);
...
// //     // If you get a 401 even with the header, the token might be invalid or the backend key changed
// //     if (err.response?.status === 401) {
// //       toast.error("Session expired. Please log in again.");
// //       navigate("/login");
// //     } else {
...
//   //   setLoading(true);
//   //   try {
//   //     // ✅ FIXED: backend route is /profile, not /user/profile
//   //     await api.post("/profile", {
//   //       ...formData,
//   //       goal: recommendedGoal,
//   //       bmi: Number(bmi.toFixed(1)),
//   //       // Logic to set default targets based on the goal
//   //       dailyCalorieIntake: recommendedGoal === "lose" ? 1800 : recommendedGoal === "gain" ? 2800 : 2200,
//   //       dailyCalorieBurn: 400,
//   //       targetSteps: 10000
//   //     });

//   //     // ✅ Refresh all data (Profile + Daily Logs) before redirecting
